...
================================================#
function calc_σ_ss(Δ, array, N, e1, drivemode)
...
function calc_total_Efield_fin(r, array, σ_ss, N, drive_type, w0, e1, forward_backward)
    ...
    if forward_backward == "forward"
        ...
        Ed_in = get_drivemode(drive_type, r, N, w0)*e1
        
        # Sum the contributions and return (in units of 1/d, i.e. return Ed)
        return Ed_in + Ed_at
        
    elseif forward_backward == "backward"
        return Ed_at
        
    else
        throw(DomainError(forward_backward, "This value for forward_backward has not been implemented in calc_total_Efield_fin"))
    end
end
...
================================================#
function calc_transmission_fin(Δ, array, N, e1, drivemode)
    # The transmission amplitude for some drive and identical detection 
    
    # Get the steady state single site coherences
    σ_ss = calc_σ_ss(Δ, array, N, e1, drivemode)
...
function calc_transmission_fin(Δ, array, drivemode, SP)
    # The transmission amplitude (or coefficient) for some drive and different choices of detection 
    # as calculated by a direct integration of the E-field (or E-field intensity) (the usual analytic expression is only for paraxial detection modes)
    
    # Get the steady state single site coherences
    σ_ss = calc_σ_ss(Δ, array, SP.N, SP.e1, drivemode)
    ...
    Ed = calc_total_Efield_fin.(integration_plane, Ref(array), Ref(σ_ss), SP.N, SP.drive_type, SP.w0, Ref(SP.e1), "forward")
    # Ed = calc_total_Efield_fin.(integration_plane, Ref(array), Ref(σ_ss), SP.N, SP.drive_type, SP.w0, Ref(SP.e1), "backward")
    
    # We get the drive, which is used in the definition of the detected light and normalization of the transmission
    drive = get_drivemode.(SP.drive_type, integration_plane, SP.N, SP.w0) .* Ref(SP.e1)
    ...
    elseif SP.detec_mode == "intensity_on_detection_plane"
        ...
        # drive = get_drivemode.(SP.drive_type, atomic_plane, SP.N, SP.w0) .* Ref(SP.e1)
        # drive_det = drive.*detection_plane
        # norm = sum(adjoint.(drive_det).*drive_det)*dx*dy
...
function calc_transmission_inf(lattice_type, N_sheets, a, L, Δ, e1, e1_label, drive_type, w0, k_n)
    ...
    for (i, kx) in enumerate(k_range), (j, ky) in enumerate(k_range)
        ...
        if drive < 1e-2*maximum(drive) continue end
...
function scan_transmission_fin(SP)
    println("Runnning scan_transmission_fin")
    ...
    if SP.detec_mode == "drive_mode"
        # Using analytic expression for detection in a paraxial mode (assumed to be the driving mode)
        ...
        # Using explicit integration of detection mode times the total E-field
        tscan = calc_transmission_fin.(reshape(SP.Delta_range, 1, SP.Delta_specs[3]), 
                                       reshape(SP.array, SP.N_inst, 1), 
                                       reshape(SP.drivemode, SP.N_inst, 1),
                                       Ref(SP))
...
function scan_transmission_inf(SP)
    println("Runnning scan_transmission_inf")
